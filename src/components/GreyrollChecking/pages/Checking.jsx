  ...
  const handleRollCheck = async (e) => {
    ...
    try {
      ...
      const submitRes = await fetch(
        "https://app.herofashion.com/corarollview/",
      );
      ...
      if (
        employeePendingRoll &&
        String(employeePendingRoll.rlno) !== String(rollNo)
      ) {
        alert(
          `You already started Roll ${employeePendingRoll.rlno}. Please submit it before checking another roll.`,
        );
        return;
      }
        ...
        <div className="w-full p-4 bg-white/70 backdrop-blur-sm rounded-xl shadow-2xl flex items-center justify-between border border-white/20 flex-col md:flex-row gap-3">
          ...
          <div className="flex flex-col space-y-4">
            ...
            <div className="text-md font-medium text-gray-700">
              <span className="font-bold text-blue-700">Roll Checker :</span>{" "}
              {allocation?.r_emp} - {allocation?.r_code}
            </div>
            <div className="text-md font-medium text-gray-700">
              <span className="font-bold text-blue-700">Helper :</span>{" "}
              {allocation?.h_emp} - {allocation?.h_code}
            </div>
          </div>
