...
const Machine = () => {
  ...
  useEffect(() => {
    ...
    if (helper.code) {
      const emp = employees.find((e) => String(e.code) === String(helper.code));
  ...
  const showSaveButton =
    rollChecker.code &&
    rollChecker.employee &&
    (!helper.code || rollChecker.code !== helper.code);

  /* ================= BORDERS (UNCHANGED) ================= */
  const rollCheckerBorder = {
    backgroundImage:
      "linear-gradient(white, white), linear-gradient(45deg, #a050ff, #ff8c00)",
  };

  const helperBorder = {
    backgroundImage:
      "linear-gradient(white, white), linear-gradient(45deg, #00cc99, #00bfff)",
  };
      ...
      <div className="flex flex-col items-center relative z-10">
        ...
        <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-8 p-10 bg-white/70 rounded-2xl shadow-3xl backdrop-blur-sm min-w-[300px] lg:min-w-[900px] border border-white/20">
          ...
          <div className="flex-1 p-8 rounded-xl shadow-lg flex flex-col space-y-4 border border-gray-100">
            ...
            <label className="text-sm md:text-lg font-medium text-gray-700 mt-4">
              Roll Checker ID
            </label>
            <input
              list="rollCheckerIds"
